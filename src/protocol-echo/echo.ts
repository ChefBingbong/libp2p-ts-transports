...
import type { EchoComponents, EchoInit, Echo as EchoInterface } from "./index.js";
...
export class Echo implements Startable, EchoInterface {
	...
	async echo(peer: PeerId | Multiaddr | Multiaddr[], buf: Uint8Array, options?: AbortOptions): Promise<Uint8Array> {
		...
		const [, output] = await Promise.all([bytes.write(buf, options), bytes.read(buf.byteLength, options)]);
