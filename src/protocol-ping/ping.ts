...
import type { AbortOptions, Logger, PeerId, Startable, Stream } from "@libp2p/interface";
...
export class PingService implements Startable, PingServiceInterface {
	...
	async ping(peer: PeerId | Multiaddr | Multiaddr[], options: AbortOptions = {}): Promise<number> {
		...
		try {
			...
			const [, result] = await Promise.all([bytes.write(data, options), bytes.read(PING_LENGTH, options)]);
