import type { ComponentLogger, Libp2pEvents, Logger, PeerId, PeerStore, TypedEventTarget } from "@libp2p/interface";
...
import { peerIdFromString } from "./peer-id";
 ...
 * to announce
 */
export type AddressFilter = (addrs: Multiaddr[]) => Multiaddr[];
...
export class AddressManager implements AddressManagerInterface {
	...
	_updatePeerStoreAddresses(): void {
		...
		const addrs = this.getAnnounceAddrs()
			...
			.concat([...this.observed.entries()].filter(([_, metadata]) => metadata.confident).map(([str]) => multiaddr(str)))
			.map((ma) => {
			...
			if (mappedIp) {
				mappedMultiaddrs.push(
					multiaddr(
						`/${tuples
							.map((tuple) => {
								return [protocols(tuple[0]).name, tuple[1]].join("/");
							})
							.join("/")}`,
					),
				);
