...
import { peerIdFromString } from "./peer-id/index.js";
...
export class DefaultUpgrader implements Upgrader {
	...
	private async _performUpgrade(
		...
		let cryptoProtocol: any;

		const upgradeTimeoutSignal = AbortSignal.timeout(
			direction === "inbound" ? this.inboundUpgradeTimeout : this.outboundUpgradeTimeout,
		);
		const signal = anySignal([upgradeTimeoutSignal, opts.signal]);
		setMaxListeners(Number.POSITIVE_INFINITY, upgradeTimeoutSignal, signal);
		opts.signal = signal;
		...
		try {
			...
			if (opts?.skipEncryption !== true) {
				...
				await this.shouldBlockConnection(
					direction === "inbound" ? "denyInboundEncryptedConnection" : "denyOutboundEncryptedConnection",
					remotePeer,
					maConn,
				);
			} else {
			...
			maConn.log.error(
				"failed to upgrade inbound connection %s %a - %e",
		...
		await this.shouldBlockConnection(
			direction === "inbound" ? "denyInboundUpgradedConnection" : "denyOutboundUpgradedConnection",
			remotePeer,
			maConn,
		);
					...
					void Promise.resolve()
						.then(async () => {
							...
							if (streamCount === incomingLimit) {
								const err = new TooManyInboundProtocolStreamsError(
									`Too many inbound protocol streams for protocol "${protocol}" - limit ${incomingLimit}`,
								);
			...
			newStream = async (protocols: string[], options: NewStreamOptions = {}): Promise<Stream> => {
				...
				try {
					if (options.signal == null) {
						muxedStream.log(
							"no abort signal was passed while trying to negotiate protocols %s falling back to default timeout",
							protocols,
						);
					...
					if (streamCount >= outgoingLimit) {
						const err = new TooManyOutboundProtocolStreamsError(
							`Too many outbound protocol streams for protocol "${protocol}" - ${streamCount}/${outgoingLimit}`,
						);
						muxedStream.abort(err);
					...
					connection.log.error(
						"could not create new outbound stream on connection %s %a for protocols %s - %e",
						direction === "inbound" ? "from" : "to",
						opts.maConn.remoteAddr,
						protocols,
						err,
					);
	...
	async _multiplexOutbound(
		...
		muxers: Map<string, StreamMuxerFactory>,
		options: AbortOptions,
	): Promise<{ stream: MultiaddrConnection; muxerFactory?: StreamMuxerFactory }> {
