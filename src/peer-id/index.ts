...
import {
	Ed25519PeerId as Ed25519PeerIdClass,
	RSAPeerId as RSAPeerIdClass,
	Secp256k1PeerId as Secp256k1PeerIdClass,
	URLPeerId as URLPeerIdClass,
} from "./peer-id";
...
export function peerIdFromMultihash(multihash: MultihashDigest): PeerId {
	...
	if (isIdentityMultihash(multihash)) {
		try {
...
export function peerIdFromCID(cid: CID): Ed25519PeerId | Secp256k1PeerId | RSAPeerId | URLPeerId {
	if (
		cid?.multihash == null ||
		cid.version == null ||
		(cid.version === 1 && cid.code !== LIBP2P_KEY_CODE && cid.code !== TRANSPORT_IPFS_GATEWAY_HTTP_CODE)
	) {
