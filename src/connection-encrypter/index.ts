...
import { InvalidCryptoExchangeError, ProtocolError, UnexpectedPeerError, serviceCapabilities } from "@libp2p/interface";
import { pbStream } from "it-protobuf-stream";
import type { Duplex } from "it-stream-types";
import type { Uint8ArrayList } from "uint8arraylist";
import { equals as uint8ArrayEquals } from "uint8arrays/equals";
import { peerIdFromPublicKey } from "../peer-id/index.js";
...
class Plaintext implements ConnectionEncrypter {
	...
	readonly [serviceCapabilities]: string[] = ["@libp2p/connection-encryption"];

	async secureInbound<Stream extends Duplex<AsyncGenerator<Uint8Array | Uint8ArrayList>> = MultiaddrConnection>(
		conn: Stream,
		options?: SecureConnectionOptions,
	): Promise<SecuredConnection<Stream>> {
		return this._encrypt(conn, options);
	}

	async secureOutbound<Stream extends Duplex<AsyncGenerator<Uint8Array | Uint8ArrayList>> = MultiaddrConnection>(
		conn: Stream,
		options?: SecureConnectionOptions,
	): Promise<SecuredConnection<Stream>> {
		return this._encrypt(conn, options);
	 ...
	 * Encrypt connection
	 */
	async _encrypt<Stream extends Duplex<AsyncGenerator<Uint8Array | Uint8ArrayList>> = MultiaddrConnection>(
		conn: Stream,
		options?: SecureConnectionOptions,
	): Promise<SecuredConnection<Stream>> {
