 ...
 * ```
 */
export async function select<Stream extends SelectStream>(
	stream: Stream,
	protocols: string | string[],
	options: MultistreamSelectInit,
): Promise<ProtocolStream<Stream>> {
 ...
 */
function optimisticSelect<Stream extends SelectStream>(
	stream: Stream,
	protocol: string,
	options: MultistreamSelectInit,
	...
	async function doSendProtocol(): Promise<void> {
		...
		try {
			...
			await lp.writeV([uint8ArrayFromString(`${PROTOCOL_ID}\n`), uint8ArrayFromString(`${protocol}\n`)]);
